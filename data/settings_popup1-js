export default class SettingsPopup {
  constructor(core) {
    ...
    // Default Settings State (kung wala sa core)
    ...
    // UI Elements State
    this.draggingSlider = null; // 'master', 'music', 'sfx', 'text'
    ...
    this.core.canvas.addEventListener("mousedown", this._clickHandler); // Changed to mousedown for dragging
  ...
  onMouseMove(e) {
    ...
    if (this.draggingSlider) {
      ...
      if (item) {
        ...
        this.settings[this.draggingSlider] = Math.round(pct * 100);
      }
    }
  }
  ...
  onClick(e) {
    ...
    L.items.forEach((item) => {
      ...
      if (
        ...
        let pct = (mx - sliderX) / item.w;
        pct = Math.max(0, Math.min(1, pct));
        this.settings[item.id] = Math.round(pct * 100);
      }
    });
  ...
  render(ctx) {
    ...
    L.items.forEach((item) => {
      ...
      // Fill (Active part)
      const val = this.settings[item.id]; // 0 to 100
    ...
    const toggleX = t.x + t.labelW;
    const isOn = this.settings.fullscreen;
